...
class Admin extends Controller {
    ...
    public function dicom_details_all() {
        ...
        if (isset($_GET['edit']) && $_GET['edit'] != "") {
            $id = $_GET['edit'];
            $data['edit'] = $this->Admindb->get_by_id('Clario', $id);
            $this->view('v2/admin/dicom/edit', $data);
        }
        if (isset($_POST['last_search'])) {
    ...
    public function Customer() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $id = $_GET['edit'];
            $data['edit'] = $this->Admindb->client_details_by_id($_GET['edit']);

            $form_data = $_POST;

            $site_code = $form_data['site_code'];
            $client_site_name = $form_data['client_site_name'];
            $address_line1 = $form_data['address_line1'];
            $address_line2 = $form_data['address_line2'];
            $city = $form_data['city'];
            $state = $form_data['state'];
            $zipcode = $form_data['zipcode'];
            $contract_tat = $form_data['contract_tat'];
            ...
            if (!empty($this->empty_key_value($form_data))) {
                ...
                $status = $this->Admindb->clientdetails_update($id,$site_code,$client_site_name,$address_line1,$address_line2,$city,$state,$zipcode,$contract_tat);
                $this->add_alert($status['type'], $status['msg']);
        ...
        if (isset($_POST['dis_submit'])) {
            ...
            if (!empty($this->empty_key_value($form_data))) {
                ...
                $user = $this->Admindb->discounted_user($client_account_id);

                if ($user) {
                    //DELETING OLD SUBS
                    $status = $this->Admindb->delete_user_discounts($client_account_id);
                }


                foreach ($minimum_value as $key => $minimum_values) {

                    $status = $this->Admindb->discount_pricing_add($client_account_id, $minimum_values, $maximum_value[$key], $percentage[$key]);
                }
            $this->add_alert($status['type'], $status['msg']);
            }
        }
        ...
        if (isset($_POST['submit_maint_fees'])) {
            ...
            if (!empty($this->empty_key_value($form_data))) {
                ...
                $status = $this->Admindb->maintenance_fees_add($form_data);
        ...
        if (isset($_POST['submit_add_rate'])) {
            ...
            if (!empty($this->empty_key_value($form_data))) {
                ...
                $user = $this->Admindb->analyses_rate_user($client_account_id);

                if ($user) { //DELETING OLD RATES
                    $status = $this->Admindb->delete_analyses_rate($client_account_id);
                }
                //INSERT NEW RATES
                foreach ($analysis_id as $key => $analysis_ids) {
                   
                    $status = $this->Admindb->analyses_rate_add($analysis_ids, $client_account_id, $rate[$key], $code[$key], $min_time[$key]);
                }
                //} // FOR EDITING
        ...
        if (isset($_POST['submit_subscription'])) {
            ...
            unset($form_datas['client_account_id']);
            ...
            $client_account_id = $this->Admindb->get_user_clientid($id);
            $total_subscription_amount = 0;

            //find total subscription
            foreach ($sub_analysis_arr as $key => $value) {
                $curr_rate = $this->Admindb->get_latest_analysis_rates($client_account_id, $sub_analysis_arr[$key]);
                
                $total_subscription_amount = $total_subscription_amount + ((int)$curr_rate * (int)$sub_count_arr[$key]);
            }
            ...
            if (empty($form_data['sub_count'])) {
                ...
                if (!empty($client_account_id)) {
                    $user = $this->Admindb->subscibed_user($client_account_id);

                    if ($user) { //DELETING OLD SUBS
                        echo "delete";
                        $status = $this->Admindb->delete_user_subscription($client_account_id);
                    }
                }
                ...
                foreach ($form_data['sub_analysis_id'] as $key => $sub_analysis_ids) {
                    ...
                    $status = $this->Admindb->subscription_add($sub_analysis_ids, $form_data['client_account_id'], $form_data['sub_count'][$key]);
                }

                if (empty($total)) {
                    
                    $this->Admindb->insert_subscription_fees($form_data['client_account_id'], $total_subscription_amount);
                }
                /* } */
        ...
        if (isset($_GET['edit']) && $_GET['edit'] != "") {
            ...
            $client_account_id = $data['edit']['client_account_id'];
            $data['analyses_rate'] = $this->Admindb->analyses_rate_user($client_account_id);

            $data['subscription'] = $this->Admindb->subscriptions_user($client_account_id);

            $data['discount_pricing_list'] = $this->Admindb->get_discount_range_by_customer($client_account_id);

         $data['max_disc'] = $this->Admindb->get_max_discount_by_customer($client_account_id);

             $data['subscription_fees'] = $this->Admindb->get_subscription_by_customer($client_account_id);
            $data['maintenance'] = $this->Admindb->fetch_all_maintenance($client_account_id);
    ...
    public function miscellaneous_billing() {
        ...
        if (isset($_GET['edit']) && $_GET['edit'] != "") {
            ...
            $data['page_title'] = 'Miscellaneous Billing';
