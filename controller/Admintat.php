...
class Admintat extends Controller
{
    ...
    public function export_turnaround_time_info_excel()
    {
        ...
        $filePath = dirname(__DIR__) . '/tmp/tat_info.xlsx';
    ...
    public function add_to_disclist()
    {
        ...
        $frm_value = $_POST['frm_value'] ?? '';
        //$to_value = $_POST['to_value'] ?? '';
        if ($_POST['to_value'] == "NaN" || trim($_POST['to_value']) === '') {
            $to_value = null;
        } else {
            $to_value = $_POST['to_value'];
        }
        ...
        if ($table_id == "parent") {
            ...
            if ($acpd_id) {
                ...
                $stmt1 = $conn->prepare($sql1);
                if (!$stmt1) {
                    echo json_encode(['status' => 'error', 'message' => 'Something went wrong. Please try again later.', 'error' => $conn->error]);
                    return;
                }
                ...
                if ($stmt1->execute()) {
                    ...
                    if ($stmt2->execute()) {
                        ...
                        echo json_encode(['status' => 'success', 'message' => 'Added to the list.']);
                    } else {
                        echo json_encode(['status' => 'error', 'message' => 'Something went wrong. Please try again later.', 'error' => $stmt2->error]);
                    }
                } else {
                    echo json_encode(['status' => 'error', 'message' => 'Something went wrong. Please try again later.', 'error' => $stmt1->error]);
                }
            } else {
    ...
    public function update_analysis_monthly_discount()
    {
        ...
        if ($_POST['analysis_to'] == "NaN" || trim($_POST['analysis_to']) === '') {
            $analysis_to = null;
        } else {
            $analysis_to = $_POST['analysis_to'];
        }
        ...
        if (
            !isset($price_id, $client_acc_id, $disc_id, $analysis_name, $analysis_from, $analysis_price) ||
            $price_id === '' ||
            $client_acc_id === '' ||
            $disc_id === '' ||
            $analysis_name === '' ||
...
LEFT JOIN analyses_client_price_details acpd 
    ON acpd.analysis_id = a.analysis_id 
    AND acpd.client_account_ids = '$cid'
INNER JOIN analyses_category ac 
    ON a.category_ids = ac.category_id
WHERE 
    ac.category_name = 'Maintenance'
    AND (
        (acpd.analysis_id IS NOT NULL AND acpd.is_active = '1')
        OR
        (acpd.analysis_id IS NULL AND a.is_active = '1')
    )
    AND COALESCE(acpd.is_deleted, a.is_deleted) != '1'";
    ...
    public function analyses_mainfee_list()
    {
        ...
        $sql = "SELECT acpd.analysis_client_price_id,
                       ...
                       acpd.analysis_id,
                       acpd.analysis_name,
                       acpd.analysis_invoicing_description,
    ...
    public function update_analysis_maintenance_fee()
    {
        ...
        $client_acc_id = $_POST['client_acc_id'] ?? '';
        $amdesc = $_POST['analysis_mdesc'] ?? '';
        $acprice = $_POST['analysis_cprice'] ?? '';
        $amprice = $_POST['analysis_mprice'] ?? '';

        if (
            !isset($aprice_id, $mainfee_id, $client_acc_id, $amdesc, $acprice, $amprice) ||
            $aprice_id === '' ||
            $mainfee_id === '' ||
            $client_acc_id === '' ||
            $amdesc === '' ||
        ...
        $update_status = $this->Tatdb->update_analysis_maintenance_fees($aprice_id, $mainfee_id, $client_acc_id, $amdesc, $acprice, $amprice);
    ...
    public function customer_excel_details()
    {
        ...
        $filename = 'Client_Details_' . ($customerDetails['client_name'] ?? 'client') . '.xlsx';
    ...
    public function export_analyses_category_info_excel()
    {
        ...
        $filePath = dirname(__DIR__) . '/tmp/analyses_category_info.xlsx';
        $writer->save($filePath);
    ...
    public function export_analyses_info_excel()
    {
        ...
        $filePath = dirname(__DIR__) . '/tmp/analyses_info.xlsx';
        $writer->save($filePath);
    ...
    public function export_client_analyses_info_excel()
    {
        ...
        $filePath = dirname(__DIR__) . '/tmp/client_analyses_info.xlsx';
        $writer->save($filePath);
    ...
    public function export_customer_info_excel()
    {
        ...
        $filePath = dirname(__DIR__) . '/tmp/customer_info.xlsx';
        $writer->save($filePath);
    ...
    AND ac.is_active = '1' AND ac.is_deleted = '0'";
        ...
        $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
        //$filePath = '/tmp/analyses_rates_info.xlsx';
        $filePath = dirname(__DIR__) . '/tmp/analyses_rates_info.xlsx';
        $writer->save($filePath);
