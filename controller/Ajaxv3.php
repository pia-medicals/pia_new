...
/*    public function get_analyst_open_worksheets_info() {
        ...
        $sql = "SELECT id, created, accession, patient_name, mrn, tat, webhook_customer, webhook_description FROM `studies` WHERE assignee = 0";
        $query = mysqli_query($con, $sql);
...
//            }
            ...
            if ($s_status == '') {
                $actData = '<a href="' . SITE_URL . '/analyst/analyst_dicom_details_my?view=' . $studies_id . '" class="btn btn-success btn-xs" >Assign</a>';
            } else if ($s_status == 'Completed') {
                $actData = '<a href="' . SITE_URL . '/analyst/analyst_dicom_details_my?view=' . $studies_id . '" class="btn btn-primary btn-xs">Edit</a><a  class="btn btn-warning btn-xs" style="display:none;" onclick="getreview(' . $studies_id . ')";>Review</a>';
            } else {
                $actData = '<a href="' . SITE_URL . '/analyst/analyst_dicom_details_my?view=' . $studies_id . '" class="btn btn-primary btn-xs">Edit</a><a  class="btn btn-warning btn-xs" style="display:none;" onclick="getreview(' . $studies_id . ')";>Review</a>';
            }
...
// deleting file
    ...
    public function update_tat_value() {
        ...
        $status = $this->Admindb->edit_tat_details($form_data);
...
public function get_analyst_open_worksheets_info() {
    ...
    $request = $_REQUEST;
    $columns = [
        'received_date', // You may need to alias or calculate this from created_at
        'accession',
        'patient_name',
        'mrn',
        'expected_time', // or default_tat if you have it
        'client_site_name',
    ...
    if(isset($request['order'])){
        $orderByCol = $columns[$request['order'][0]['column']];
        $orderDir = $request['order'][0]['dir'];
        $order = "ORDER BY $orderByCol $orderDir";
    }

    // Filtering (add your own search logic as needed)
    $where = "WHERE is_deleted = 0";
    if(!empty($request['search']['value'])){
        $search = $con->real_escape_string($request['search']['value']);
        $where .= " AND (accession LIKE '%$search%' OR patient_name LIKE '%$search%' OR mrn LIKE '%$search%')";
    }

    // Total records
    $sql_total = "SELECT COUNT(*) as cnt FROM studies $where";
    $res_total = $con->query($sql_total);
    $totalData = $res_total->fetch_assoc()['cnt'];

    // Data query
    $sql = "SELECT studies_id, created_at as received_date, accession, patient_name, mrn, expected_time, client_site_name, comment 
            FROM studies $where $order $limit";
    $query = $con->query($sql);

    $data = [];
    while ($row = $query->fetch_assoc()) {
        $subdata = [];
        $subdata[] = date("m-d-Y h:i:s A", strtotime($row['received_date']));
        $subdata[] = htmlspecialchars($row['accession']);
        $subdata[] = htmlspecialchars($row['patient_name']);
        $subdata[] = htmlspecialchars($row['mrn']);
        $subdata[] = htmlspecialchars($row['expected_time']);
        $subdata[] = htmlspecialchars($row['client_site_name']);
        $subdata[] = htmlspecialchars($row['comment']);
        $subdata[] = '<a href="' . SITE_URL . '/dashboard/open_work_sheets?view=' . $row['studies_id'] . '" class="btn btn-primary btn-xs">View</a>';
        $data[] = $subdata;
    }
