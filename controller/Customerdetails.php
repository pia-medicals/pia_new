...
class Customerdetails extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->Customerdb = $this->model('customerdb');

        $this->Ajax = $this->model('ajaxdb');
        $this->Analyst = $this->model('analystratemodel');
        $this->Assigndb = $this->model('adminassignmodel');
        $this->Admindb_v1 = $this->model('admindb_v1');

        if (isset($_SESSION['user']) && $_SESSION['user']->user_type_ids == 1 || $_SESSION['user']->user_type_ids == 3 || $_SESSION['user']->user_type_ids == 5) {
...
Assigned')) . '">Not Assigned</span>';
    ...
    public function get_all_worksheet_info()
    {
        ...
        $sql1 = "SELECT studies.studies_id, studies.created_at, studies.accession, studies.patient_name, studies.mrn, studies.actual_tat as dtat, users.user_name as name, studies.status_ids, studies.second_analyst_id, studies.client_account_ids as assign_customer, studies.updated_at, studies.dicom_webhook_ids,dicom_webhook_details.webhook_customer,dicom_webhook_details.webhook_description FROM studies LEFT JOIN users on studies.analyst_id = users.user_id LEFT JOIN dicom_webhook_details on studies.dicom_webhook_ids = dicom_webhook_details.dicom_webhook_id WHERE 1=1";
        ...
        //  $sql1 .= " ORDER BY " . $col[$request['order'][0]['column']] . "   " . $request['order'][0]['dir'] . "  LIMIT " .
        //$request['start'] . "  ," . $request['length'] . "  ";
...
Assigned')) . '">Not Assigned</span>';
    ...
    public function customer_details($customer_id) // Add customer_id parameter
    {
        $con = $this->getConnection();

        // Sanitize the customer ID to prevent SQL injection (important!)
        $customer_id = mysqli_real_escape_string($con, $customer_id);

        $sql = "SELECT studies.studies_id, studies.accession, studies.mrn, studies.patient_name, studies.analyst_id, studies.second_analyst_id, studies.dicom_webhook_ids, studies.status_ids, studies.created_at, studies.actual_tat, studies.client_account_ids, dicom_webhook_details.dicom_webhook_id, dicom_webhook_details.webhook_customer, dicom_webhook_details.webhook_description FROM studies LEFT JOIN dicom_webhook_details ON studies.dicom_webhook_ids = dicom_webhook_details.dicom_webhook_id WHERE studies.client_account_ids = '" . $customer_id . "'";
        ...
        if ($query && mysqli_num_rows($query) > 0) {
            ...
            if (!empty($originalDate)) {
                $data['received_date'] = $newDate;
                $data['received_time'] = $newTime;
            } else {
                $data['received_date'] = $row[8];
                $data['received_time'] = $row[8];
            }
            $data['accession'] = $row[1];
            $data['patient_name'] = $row[3];
            $data['tat'] = $row[9];
            $data['webhook_customer'] = $row[12];
            $first_ananlyst_name = $this->Admindb->get_user_by_id($row[4]);
            $data['first_analyst'] = $first_ananlyst_name;
            $second_ananlyst_name = $this->Admindb->get_user_by_id($row[5]);
            $data['second_analyst'] = $second_ananlyst_name;
            $client_details = $this->Admindb->get_client_details_by_id($row[10]);
            $data['client_name'] = $client_details['client_name'];
            $data['webhook_description'] = $row[13];
            $status = $row[7];
            if (!empty($status)) {
    ...
    public function customer_wise_details()
    {
        $con = $this->getConnection();
        $request = $_REQUEST;
        $col = array(
    ...
    //First success
    // public function single_customer_details()
    // {
    ...
    //     $client_accid = $this->Customerdb->getClientaccId($user_id);

    //     if (!$client_accid) {
    //         header('Content-Type: application/json');
    //         echo json_encode(["error" => "Client account ID not found"]);
    //         exit;
    //     }

    //     $customer_name = $_SESSION['user']->user_name;
    ...
    //     $totalRecords = $this->Customerdb->getTotalStudyCount($client_accid);

    //     $studyData = $this->Customerdb->getStudyData($client_accid, $start, $length);
    ...
    //         if (!empty($originalDate)) {
    //             $row['received_date'] = date("m-d-Y", strtotime($originalDate));
    //             $row['received_time'] = date("h:i", strtotime($originalDate));
    //         } else {
    //             $row['received_date'] = '';
    //             $row['received_time'] = '';
    //         }
    ...
    //         if (!empty($assigneeID)) {
    //             $assigneeName = $this->Admindb->get_user_by_id($assigneeID);
    //             $row['assignee'] = $assigneeName;
    //         }

    //         //Second Check Name
    //         $secondCheckID = $row['second_check'];
    //         if (!empty($secondCheckID)) {
    //             $secondCheckName = $this->Admindb->get_user_by_id($secondCheckID);
    //             $row['second_check'] = $secondCheckName;
    //         }

    //         //Status
    //         $statusID = $row['status'];
    //         if (!empty($statusID)) {
    //             $status = $this->Admindb->get_status_details($statusID);
    //             $row['status'] = $status;
    //         } else {
    ...
    //             // "default_tat"      => $row['actual_tat'],
    //             "default_tat"      => $row['default_tat'],
    //             "webhook_customer" => $row['webhook_customer'],
    ...
    public function single_customer_details()
    {
        ...
        $assignee = isset($_POST['assignee']) ? intval($_POST['assignee']) : '';
        $second_assignee = isset($_POST['second_assignee']) ? intval($_POST['second_assignee']) : '';
        $status = isset($_POST['status']) ? intval($_POST['status']) : '';
        $search_value = isset($_POST['search_value']) ? trim($_POST['search_value']) : '';

        // Remove hidden characters
        $search_value = preg_replace('/[^\x20-\x7E]/', '', $search_value);

        // Convert to UTF-8 to avoid encoding issues
        $search_value = mb_convert_encoding($search_value, 'UTF-8', 'UTF-8');

        // echo "Cleaned Search Value: " . $search_value;
        // exit;
        ...
        $studyData = $this->Customerdb->getStudyData($client_accid, $start, $length, $days, $assignee, $second_assignee, $status,  $search_value);

        $data = [];
        foreach ($studyData as $row) {
            $originalDate = $row['original_date'];
            $row['received_date'] = !empty($originalDate) ? date("m-d-Y", strtotime($originalDate)) : '';
            $row['received_time'] = !empty($originalDate) ? date("h:i", strtotime($originalDate)) : '';

            $row['assignee'] = !empty($row['assignee']) ? $this->Admindb->get_user_by_id($row['assignee']) : '';
            $row['second_check'] = !empty($row['second_check']) ? $this->Admindb->get_user_by_id($row['second_check']) : '';
            $row['status'] = !empty($row['status']) ? $this->Admindb->get_status_details($row['status']) : "NOT STARTED";
        ...
        $response = [
            "draw" => isset($_POST['draw']) ? intval($_POST['draw']) : 1,
            "recordsTotal" => $totalRecords,
            // "recordsFiltered" => count($data), // Count after filtering
            "recordsFiltered" => $this->Customerdb->getFilteredStudyCount($client_accid, $days, $assignee, $second_assignee, $status, $search_value),
            "data" => $data
        ];
    ...
    public function single_customer_details_csv()
    {
        if (!isset($_SESSION['user']->user_id)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Unauthorized access"]);
        ...
        //Length for CSV
        $length = $this->Customerdb->getFilteredStudyCount($client_accid, $days, $assignee, $second_assignee, $status, $search_value);
        ...
        $columnHeader = "SL No" . "\t" . "Received Date" . "\t" . "Received Time" . "\t" . "Accession" . "\t" . "Patient Name" . "\t" . "Default TAT" . "\t" . "Webhook Customer" . "\t" . "Assignee" . "\t" . "Second Check" . "\t" . "Customer" . "\t" . "Description" . "\t" . "Status" . "\t";
        $setData = '';
        $i = 0;
        $user_arr = array();
        $user_arr[] = array("SL No", "Received Date", "Received Time", "Accession", "Patient Name", "Default TAT", "Webhook Customer", "Assignee", "Second Check", "Customer", "Description", "Status");
        ...
        foreach ($studyData as $row) {
            ...
            $user_arr[] = array($i, $row['received_date'], $row['received_time'], $row['accession'], $row['patient_name'], $row['default_tat'], $row['webhook_customer'], $row['assignee'], $row['second_check'], $customer_name, $row['description'], $row['status']);
    ...
    // public function single_customer_details_xlsx()
    // {
    //     if (!isset($_SESSION['user']->user_id)) {
    //         header('Content-Type: application/json');
    //         echo json_encode(["error" => "Unauthorized access"]);
    ...
    //     $columnHeaders = ["SL No", "Received Date", "Received Time", "Accession", "Patient Name", "Default TAT", "Webhook Customer", "Assignee", "Second Check", "Customer", "Description", "Status"];
    //     $sheet->fromArray([$columnHeaders], null, 'A1');

    //     $i = 2; // Start from row 2 (after headers)
    //     foreach ($studyData as $row) {
    //         $originalDate = $row['original_date'];
    //         $row['received_date'] = !empty($originalDate) ? date("m-d-Y", strtotime($originalDate)) : '';
    //         $row['received_time'] = !empty($originalDate) ? date("h:i", strtotime($originalDate)) : '';

    //         $assigneeName = !empty($row['assignee']) ? $this->Admindb->get_user_by_id($row['assignee']) : '';
    //         $secondCheckName = !empty($row['second_check']) ? $this->Admindb->get_user_by_id($row['second_check']) : '';
    //         $statusName = !empty($row['status']) ? $this->Admindb->get_status_details($row['status']) : "NOT STARTED";
    ...
    public function single_customer_details_excel()
    {
        if (!isset($_SESSION['user']->user_id)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Unauthorized access"]);
        ...
        $days = $_POST['days'] ?? '';
        $assignee = $_POST['assignee'] ?? '';
        $second_assignee = $_POST['second_assignee'] ?? '';
        $status = $_POST['status'] ?? '';
        ...
        $spreadsheet->getProperties()
            ...
            ->setTitle('Customer Studies Report')
            ->setDescription('Generated Excel report using PhpSpreadsheet.');

        $headers = ["SL No", "Received Date", "Received Time", "Accession", "Patient Name", "Default TAT", "Webhook Customer", "Assignee", "Second Check", "Customer", "Description", "Status"];
        ...
        $rowNumber = 2;
        foreach ($studyData as $index => $row) {
            $receivedDate = !empty($row['original_date']) ? date("m-d-Y", strtotime($row['original_date'])) : '';
            $receivedTime = !empty($row['original_date']) ? date("h:i", strtotime($row['original_date'])) : '';
            $assigneeName = !empty($row['assignee']) ? $this->Admindb->get_user_by_id($row['assignee']) : '';
            $secondCheck = !empty($row['second_check']) ? $this->Admindb->get_user_by_id($row['second_check']) : '';
            $statusText = !empty($row['status']) ? $this->Admindb->get_status_details($row['status']) : "NOT STARTED";
        ...
        $filePath = '/tmp/Studies_Report.xlsx';
        ...
        ob_end_clean();
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="Studies_Report.xlsx"');
    ...
    public function getAssignees()
    {
        ...
        $response = $this->Customerdb->fetchAssignees($search, $page);

        header('Content-Type: application/json');
        echo json_encode($response);
    }

    public function getSecondAssignees()
    {
        $search = $_GET['search'] ?? '';
        $page = $_GET['page'] ?? 1;

        // $model = new customerdb();
        $response = $this->Customerdb->fetchAssignees($search, $page); // Same logic as assignees
    ...
    public function getStatuses()
    {
        ...
        // $model = new customerdb();
        $response = $this->Customerdb->fetchStatuses($search, $page);

        header('Content-Type: application/json');
        echo json_encode($response);
    }

    public function single_customer_stat_info()
    {
        if (!isset($_SESSION['user']->user_id)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Unauthorized access"]);
        ...
        $user_id = $_SESSION['user']->user_id;
        $client_accsitecode = $this->Customerdb->getClientaccId_siteCode($user_id);

        if (!$client_accsitecode) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Client account ID not found"]);
            exit;
        } else {
            $client_accid = $client_accsitecode['client_accid'];
            $client_id = $client_accsitecode['client_id'];
            $site_code = $client_accsitecode['site_code'];
        }

        $client_numname = $this->Customerdb->getClientName($client_id);

        if (empty($client_numname) || !is_array($client_numname)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Client number and name not found"]);
            exit;
        }

        $client_number = isset($client_numname['client_number']) ? $client_numname['client_number'] : '';
        $client_name = isset($client_numname['client_name']) ? $client_numname['client_name'] : '';
        ...
        // Fetch total records before filtering
        $totalRecords = $this->Customerdb->getTotalStatCount($client_accid);

        // Fetch filtered records
        $statData = $this->Customerdb->getStatData($client_accid, $start, $length, $days, $assignee, $second_assignee, $status,  $search_value);
        ...
        foreach ($statData as $row) {
            ...
            $row['completed_date'] = !empty($completedTime) ? date("m-d-Y", strtotime($completedTime)) : '';
            $row['completed_time'] = !empty($completedTime) ? date("h:i", strtotime($completedTime)) : '';

            $row['assignee'] = !empty($row['assignee']) ? $this->Admindb->get_user_by_id($row['assignee']) : '';
            $row['second_check'] = !empty($row['second_check']) ? $this->Admindb->get_user_by_id($row['second_check']) : '';
            $analysis_details = !empty($row['analysis_performed_id']) ? $this->Admindb->get_analysis_details_by_id($row['analysis_performed_id']) : [];

            $analysis_name = isset($analysis_details['analysis_name']) ? $analysis_details['analysis_name'] : '';
            $analysis_code = isset($analysis_details['analysis_code']) ? $analysis_details['analysis_code'] : '';
            $item_number = isset($analysis_details['analysis_id']) ? $analysis_details['analysis_id'] : '';

            $analysis_number = !empty($item_number) ? $this->Admindb->get_analysis_number_by_id($item_number) : '';
            ...
            $data[] = [
                ...
                "site_code"     => $site_code,
                "analysis_performed" => $analysis_name,
                "pia_analysis_code" => $analysis_code,
                "item_numbers" => $analysis_number,
                "status" => $row['status'],
        ...
        $response = [
            "draw" => isset($_POST['draw']) ? intval($_POST['draw']) : 1,
            "recordsTotal" => $totalRecords,
            "recordsFiltered" => $this->Customerdb->getFilteredStatCount($client_accid, $days, $assignee, $second_assignee, $status, $search_value),
            "data" => $data
        ];
    ...
    public function single_customer_stat_details_excel()
    {
        if (!isset($_SESSION['user']->user_id)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Unauthorized access"]);
        ...
        $client_number = $client_numname['client_number'] ?? '';
        $client_name = $client_numname['client_name'] ?? '';
        $customer_name = $_SESSION['user']->user_name;
        ...
        $totalRecords = $this->Customerdb->getTotalStatCount($client_accid);
        $length = $this->Customerdb->getFilteredStatCount($client_accid, $days, $assignee, $second_assignee, $status, $search_value);
        $statData = $this->Customerdb->getStatData($client_accid, $start, $length, $days, $assignee, $second_assignee, $status, $search_value);
        ...
        $spreadsheet->getProperties()
            ->setCreator('Dicon')
            ->setLastModifiedBy('Dicon')
            ->setTitle('Customer Stats Report')
            ->setDescription('Generated Excel report using PhpSpreadsheet.');

        $headers = ["SL No", "Received Date", "Received Time", "Accession", "Assignee", "Second Check", "Client Name", "Client Number", "Site Code", "Analysis Performed", "PIA Analysis Code", "Item Numbers", "Status", "Completed Date", "Completed Time"];
        $sheet->fromArray([$headers], NULL, 'A1');
        ...
        $rowNumber = 2;
        foreach ($statData as $index => $row) {
            $receivedDate = !empty($row['original_date']) ? date("m-d-Y", strtotime($row['original_date'])) : '';
            $receivedTime = !empty($row['original_date']) ? date("h:i", strtotime($row['original_date'])) : '';
            $completedDate = !empty($row['completed_time']) ? date("m-d-Y", strtotime($row['completed_time'])) : '';
            $completedTime = !empty($row['completed_time']) ? date("h:i", strtotime($row['completed_time'])) : '';
            $assigneeName = !empty($row['assignee']) ? $this->Admindb->get_user_by_id($row['assignee']) : '';
            $secondCheck = !empty($row['second_check']) ? $this->Admindb->get_user_by_id($row['second_check']) : '';
            $analysis_details = !empty($row['analysis_performed_id']) ? $this->Admindb->get_analysis_details_by_id($row['analysis_performed_id']) : [];

            $analysisName = $analysis_details['analysis_name'] ?? '';
            $analysisCode = $analysis_details['analysis_code'] ?? '';
            $itemNumber = $analysis_details['analysis_id'] ?? '';
            $analysisNumber = !empty($itemNumber) ? $this->Admindb->get_analysis_number_by_id($itemNumber) : '';
            $statusText = !empty($row['status']) ? $this->Admindb->get_status_details($row['status']) : "NOT STARTED";
    ...
    public function single_studies_info()
    {
        $con = $this->getConnection();
        $request = $_REQUEST;
        // $col = array(
        ...
        $col = array(
            0 => 'studies.created_at', // Received Date
            1 => 'studies.created_at', // Received Time
            2 => 'studies.accession',
            3 => 'studies.patient_name',
            4 => 'studies.mrn',
            5 => 'studies.actual_tat',
            6 => 'dicom_webhook_details.webhook_customer',
            7 => 'studies.analyst_id',
            8 => 'studies.second_analyst_id',
            9 => 'studies.client_account_ids',
            10 => 'dicom_webhook_details.webhook_description',
            11 => 'studies.status_ids'
        );
        ...
        //$sql = "SELECT studies.studies_id,studies.accession,  studies.mrn, studies.patient_name, studies.analyst_id,studies.second_analyst_id ,  studies.dicom_webhook_ids, studies.status_ids , studies.created_at,  studies.actual_tat,studies.client_account_ids,dicom_webhook_details.dicom_webhook_id,dicom_webhook_details.webhook_customer ,dicom_webhook_details.webhook_description FROM studies LEFT JOIN dicom_webhook_details on studies.dicom_webhook_ids = dicom_webhook_details.dicom_webhook_id WHERE s.client_account_ids = '" . $client_accid . "' ";
        $sql = "SELECT studies.studies_id, studies.accession, studies.mrn, studies.patient_name, 
               studies.analyst_id, studies.second_analyst_id, studies.dicom_webhook_ids, 
               studies.status_ids, studies.created_at, studies.actual_tat, studies.client_account_ids, 
               dicom_webhook_details.dicom_webhook_id, dicom_webhook_details.webhook_customer, 
               dicom_webhook_details.webhook_description 
        FROM studies 
        LEFT JOIN dicom_webhook_details 
        ON studies.dicom_webhook_ids = dicom_webhook_details.dicom_webhook_id 
        WHERE studies.client_account_ids = '" . $client_accid . "'";
        ...
        if (!empty($request['search']['value'])) {
            ...
            $sql .= " OR studies.patient_name LIKE '%" . $request['search']['value'] . "%' ";
            $sql .= " OR studies.client_site_name LIKE '%" . $request['search']['value'] . "%' ";
            $sql .= " OR studies.comment LIKE '%" . $request['search']['value'] . "%' ";
            $sql .= " OR studies.actual_tat LIKE '%" . $request['search']['value'] . "%' ";
            $sql .= " OR dicom_webhook_details.webhook_customer LIKE '%" . $request['search']['value'] . "%' )";
        }
        ...
        // $orderByColumn = $col[$orderColumnIndex] ?? 'studies.studies_id';
        ...
        $totalFilterQuery = mysqli_query($con, "SELECT COUNT(*) as total FROM ($sql) as subquery");
        $totalFilterRow = mysqli_fetch_assoc($totalFilterQuery);
        $totalFilter = $totalFilterRow['total'];

        $sql .= " ORDER BY $orderByColumn $orderDir LIMIT $start, $length";
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            if (!empty($row[9])) {
                ...
                $contract_tat = $this->Admindb_v1->get_tat_details_by_id($row[10]);
                $subdata[] = $contract_tat;
            }

            $subdata[] = $row[12];

            $first_analyst_name = $this->Admindb->get_user_by_id($row[4]);
            $subdata[] = $first_analyst_name;

            $second_analyst_name = $this->Admindb->get_user_by_id($row[5]);
            $subdata[] = $second_analyst_name;
            ...
            $status = $row[7];
            $study_status = !empty($status) ? $this->Admindb->get_status_details($status) : "NOT STARTED";
            $subdata[] = $study_status;
    ...
    public function single_studies_info_excel()
    {
        if (!isset($_SESSION['user']->user_id)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Unauthorized access"]);
        ...
        $status = $_POST['status'] ?? '';
        $searchValue = $_POST['searchValue'] ?? ''; // Retrieve search value
        ...
        if (!empty($searchValue)) {
            $sql .= " AND (studies.accession LIKE '%" . $searchValue . "%' ";
            $sql .= " OR studies.mrn LIKE '%" . $searchValue . "%' ";
            $sql .= " OR studies.patient_name LIKE '%" . $searchValue . "%' ";
            $sql .= " OR studies.client_site_name LIKE '%" . $searchValue . "%' ";
            $sql .= " OR studies.comment LIKE '%" . $searchValue . "%' ";
            $sql .= " OR studies.actual_tat LIKE '%" . $searchValue . "%' ";
            $sql .= " OR dicom_webhook_details.webhook_customer LIKE '%" . $searchValue . "%' )";
        }

        // Notice that there is no LIMIT and ORDER clause here.
        ...
        $spreadsheet->getProperties()
            ->setCreator('Dicon')
            ->setLastModifiedBy('Dicon')
            ->setTitle('Studies Report')
            ->setDescription('Generated Excel report using PhpSpreadsheet.');

        $headers = ["Received Date", "Received Time", "Accession", "Patient Name", "MRN", "Default TAT", "Webhook Customer", "Assignee", "Second Check", "Client", "Description", "Status"];
        $sheet->fromArray([$headers], NULL, 'A1');
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            $defaultTAT = !empty($row[9]) ? $row[9] : $this->Admindb_v1->get_tat_details_by_id($row[10]);
            $firstAnalystName = $this->Admindb->get_user_by_id($row[4]);
            $secondAnalystName = $this->Admindb->get_user_by_id($row[5]);
            $clientDetails = $this->Admindb->get_client_details_by_id($row[10]);
            $clientName = $clientDetails['client_name'];
            $studyStatus = !empty($row[7]) ? $this->Admindb->get_status_details($row[7]) : "NOT STARTED";
    ...
    public function single_stat_info()
    {
        $con = $this->getConnection();
        $request = $_REQUEST;
        ...
        $col = array(
            ...
            1 => 'studies.created_at',
            2 => 'studies.accession',
            3 => 'studies.analyst_id',
            4 => 'studies.second_analyst_id',
            5 => 'studies.client_account_ids',
            6 => 'studies.client_account_ids',
            7 => 'studies.client_account_ids',
            8 => 'analyses_performed.analysis_performed_id',
            9 => 'analyses_performed.analysis_performed_id',
            10 => 'analyses_performed.analysis_performed_id',
            11 => 'studies.status_ids',
            12 => 'studies.completed_time',
            13 => 'studies.completed_time'
        );  //create column like table in database
        ...
        $sql = "SELECT studies.studies_id, studies.client_account_ids, studies.accession, studies.mrn, studies.patient_name, studies.client_site_name, studies.analyst_id, studies.second_analyst_id , studies.dicom_webhook_ids, studies.status_ids , studies.created_at, studies.completed_time, studies.actual_tat, studies.client_account_ids, studies.client_site_name, analyses_performed.analysis_performed_id, analyses_performed.studies_ids FROM studies JOIN analyses_performed on studies.studies_id = analyses_performed.studies_ids WHERE studies.client_account_ids = '" . $client_accid . "'";
        ...
        if (!empty($request['search']['value'])) {
            ...
            $sql .= " OR studies.patient_name LIKE '%" . $request['search']['value'] . "%' ";
            $sql .= " OR studies.client_site_name LIKE '%" . $request['search']['value'] . "%' ";
            $sql .= " OR studies.created_at LIKE '%" . $request['search']['value'] . "%' ";
            $sql .= " OR studies.completed_time LIKE '%" . $request['search']['value'] . "%' )";
        }
    ...
    public function single_stat_info_excel()
    {
        if (!isset($_SESSION['user']->user_id)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Unauthorized access"]);
        ...
        if (!empty($searchValue)) {
            ...
            $sql .= " OR studies.patient_name LIKE '%" . $searchValue . "%' ";
            $sql .= " OR studies.client_site_name LIKE '%" . $searchValue . "%' ";
            $sql .= " OR studies.completed_time LIKE '%" . $searchValue . "%' )";
        }
        ...
        $headers = ["Received Date", "Received Time", "Accession", "Assignee", "Second Check", "Client Name", "Client Number", "Site Code", "Analysis Perfomed", "Pia Analysis Code", "Item Numbers", "Status", "Completed Date", "Completed Time"];
        $sheet->fromArray([$headers], NULL, 'A1');
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            $firstAnalystName = $this->Admindb->get_user_by_id($row[6]);
            $secondAnalystName = $this->Admindb->get_user_by_id($row[7]);
            $clientDetails = $this->Admindb->get_client_details_by_id($row[13]);
            $site_code =  $this->Admindb->get_user_sitecode($row[13]);
            $analysis_details = $this->Admindb->get_analysis_details_by_id($row[15]);
            $item_number = $analysis_details['analysis_id'];
            $analysis_number = $this->Admindb->get_analysis_number_by_id($item_number);
            $status_id = $row[9];
            if (!empty($status_id)) {
                $study_status = $this->Admindb->get_status_details($status_id);
            } else {
                $study_status = "NOT STARTED";
            }
            ...
            $newTime_completed = date("h:i", strtotime($completeDate));
            if (!empty($completeDate)) {
                $completedDate = $newDate_completed;
                $completedTime = $newTime_completed;
            } else {
                $completedDate = $row[11];
                $completedTime = $row[11];
            }
            ...
            $dataRow = [
                ...
                $site_code,
                $analysis_details['analysis_name'],
                $analysis_details['analysis_code'],
                $analysis_number,
                $study_status,
                $completedDate,
                $completedTime,
    ...
    public function profile()
    {
        ...
        $this->view('v2/customer/customer_profile', $data);
    }

    public function new_single_stat_info_old()
    {
        ini_set('display_errors', 1);
        ...
        //     6 => 'studies.client_account_ids',
        //     7 => 'studies.client_account_ids',
        //     8 => 'analyses_client_price_details.analysis_name',
        //     9 => 'analyses_client_price_details.analysis_code',
        //     10 => 'analyses.analysis_number',
        //     11 => 'studies.status_ids',
        //     12 => 'studies.completed_time',
        //     13 => 'studies.completed_time'
        // );
        ...
        $col = array(
            ...
            3 => 'studies.analyst_id',
            4 => 'studies.second_analyst_id',
            5 => 'studies.client_site_name',
            6 => 'clients.client_number',
            7 => 'client_details.site_code',
            8 => 'analyses_performed.analysis_performed',
            9 => 'analyses_client_price_details.analysis_code',
            10 => 'analyses.analysis_number',
            11 => 'studies.status_ids',
            12 => 'studies.completed_time',
        ...
        // $sql = "SELECT 
        //             studies.studies_id, 
        //             studies.client_account_ids, 
        //             studies.accession, 
        //             studies.client_site_name, 
        //             studies.analyst_id, 
        //             studies.second_analyst_id, 
        //             studies.status_ids, 
        //             studies.created_at, 
        //             studies.completed_time, 
        //             analyses_performed.analysis_performed_id, 
        //             analyses_performed.studies_ids, 
        //             analyses_client_price_details.analysis_id, 
        //             analyses_client_price_details.analysis_name, 
        //             analyses_client_price_details.analysis_code, 
        //             analyses.analysis_number 
        //         FROM studies
        //         JOIN analyses_performed 
        //             ON studies.studies_id = analyses_performed.studies_ids
        //         JOIN analyses_client_price_details 
        //             ON analyses_client_price_details.client_account_ids = studies.client_account_ids
        //         JOIN analyses 
        //             ON analyses_client_price_details.analysis_id = analyses.analysis_id
        //         WHERE studies.client_account_ids = '" . $client_accid . "'";
        ...
        //             studies.created_at, 
        //             studies.completed_time, 
        //             client_details.client_ids, 
        //             client_details.site_code, 
        //             clients.client_number,
        //             analyses_performed.analysis_client_price_ids,
        //             analyses_performed.analysis_performed,
        //             analyses_client_price_details.analysis_id,
        //             analyses_client_price_details.analysis_code,
        //             analyses.analysis_number 
        //         FROM studies
        //         JOIN client_details
        //         ON client_details.client_account_id = '" . $client_accid . "'
        //         JOIN clients
        //             ON client_details.client_ids = clients.client_id
        //         JOIN analyses_performed
        //             ON studies.studies_id = analyses_performed.studies_ids
        //         JOIN analyses_client_price_details
        //             ON analyses_performed.analysis_client_price_ids = analyses_client_price_details.analysis_client_price_id
        //         JOIN analyses
        //             ON analyses_client_price_details.analysis_id = analyses.analysis_id
        //         WHERE studies.client_account_ids = '" . $client_accid . "'";
        ...
        $sql .= " ORDER BY studies.studies_id " . $request['order'][0]['dir'] . "  
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            $newDate = !empty($originalDate) ? date("m-d-Y", strtotime($originalDate)) : $row['created_at'];
            $newTime = !empty($originalDate) ? date("h:i", strtotime($originalDate)) : $row['created_at'];
            ...
            // Fetch Analyst Names
            $subdata[] = $this->Admindb->get_user_by_id($row['analyst_id']);
            $subdata[] = $this->Admindb->get_user_by_id($row['second_analyst_id']);
            ...
            // Fetch Site Code
            $subdata[] = $this->Admindb->get_user_sitecode($row['client_account_ids']);

            // Fetch Analysis Details
            $subdata[] = $row['analysis_name'];
            $subdata[] = $row['analysis_code'];

            // Fetch Analysis Number
            $subdata[] = $row['analysis_number'];

            // Fetch Study Status
            $study_status = !empty($row['status_ids']) ?
                $this->Admindb->get_status_details($row['status_ids']) : "NOT STARTED";
            $subdata[] = $study_status;

            // Formatting completed_time
            $completeDate = $row['completed_time'];
            $newDate_completed = !empty($completeDate) ? date("m-d-Y", strtotime($completeDate)) : $row['completed_time'];
            $newTime_completed = !empty($completeDate) ? date("h:i", strtotime($completeDate)) : $row['completed_time'];
    ...
    public function new_single_stat_info()
    {
        ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(E_ALL);
        ...
        $sql = "SELECT 
            ...
            FROM studies
            JOIN client_details
                ON client_details.client_account_id = studies.client_account_ids
            JOIN clients
                ON client_details.client_ids = clients.client_id
            JOIN analyses_performed
                ON studies.studies_id = analyses_performed.studies_ids
        ...
        // if (!empty($request['search']['value'])) {
        //     $searchValue = mysqli_real_escape_string($con, $request['search']['value']);
        //     $sql .= " AND (studies.accession LIKE '%$searchValue%' 
        //                 OR studies.mrn LIKE '%$searchValue%' 
        //                 OR studies.patient_name LIKE '%$searchValue%' 
        //                 OR studies.client_site_name LIKE '%$searchValue%' 
        //                 OR studies.created_at LIKE '%$searchValue%' 
        //                 OR studies.completed_time LIKE '%$searchValue%')";
        // }
        ...
        if (!empty($request['search']['value'])) {
            ...
            $sql .= " AND (studies.accession LIKE '%$searchValue%' 
                    ...
                    OR studies.created_at LIKE '%$searchValue%' 
                    OR studies.completed_time LIKE '%$searchValue%' 
                    OR analyses_client_price_details.analysis_name LIKE '%$searchValue%' 
                    OR analyses_client_price_details.analysis_code LIKE '%$searchValue%' 
                    OR analyses.analysis_number LIKE '%$searchValue%')";
        }
        ...
        $orderColumn = isset($request['order'][0]['column']) ? $col[$request['order'][0]['column']] : 'studies.studies_id';
        $orderDir = isset($request['order'][0]['dir']) ? mysqli_real_escape_string($con, $request['order'][0]['dir']) : 'ASC';
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            $originalDate = $row['created_at'];
            $newDate = !empty($originalDate) ? date("m-d-Y", strtotime($originalDate)) : '';
            $newTime = !empty($originalDate) ? date("h:i A", strtotime($originalDate)) : '';
            ...
            $study_status = !empty($row['status_ids']) ? $this->Admindb->get_status_details($row['status_ids']) : "NOT STARTED";
            $subdata[] = $study_status;

            $completeDate = $row['completed_time'];
            $newDate_completed = !empty($completeDate) ? date("m-d-Y", strtotime($completeDate)) : '';
            $newTime_completed = !empty($completeDate) ? date("h:i A", strtotime($completeDate)) : '';
    ...
    public function new_single_stat_info_excel()
    {
        if (!isset($_SESSION['user']->user_id)) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Unauthorized access"]);
        ...
        $secondAssignee = $_POST['secondAssignee'] ?? '';
        $status = $_POST['status'] ?? '';
        //$searchValue = $_POST['search']['value'] ?? ''; // Retrieve search value
        $searchValue = $_POST['searchValue'] ?? ''; // Retrieve search value
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            $firstAnalystName = $this->Admindb->get_user_by_id($row['analyst_id']);
            $secondAnalystName = $this->Admindb->get_user_by_id($row['second_analyst_id']);
    ...
    public function new_single_stat_info_sort(){
        $con = $this->getConnection();
        $request = $_REQUEST ?? [];
        ...
        $sql = "SELECT 
                ...
                analyses_client_price_details.analysis_code,
                analyses.analysis_number,
                ua.user_name as analyst_name,
                usb.user_name as second_analyst_name
            FROM studies
JOIN client_details
    ON client_details.client_account_id = studies.client_account_ids
JOIN clients
...
JOIN analyses
    ON analyses_client_price_details.analysis_id = analyses.analysis_id
JOIN users ua
    ON studies.analyst_id = ua.user_id
JOIN users usb
    ON studies.second_analyst_id = usb.user_id
            WHERE studies.client_account_ids = '$client_accid'";
...
$query = mysqli_query($con, $sql);
        ...
        while ($row = mysqli_fetch_array($query)) {
            ...
            // $subdata[] = $this->Admindb->get_user_by_id($row['analyst_id']);
            // $subdata[] = $this->Admindb->get_user_by_id($row['second_analyst_id']);
            $subdata[] = $row['analyst_name'];
            $subdata[] = $row['second_analyst_name'];
    ...
    // public function new_single_stat_infoo()
    // {
    //     ini_set('display_errors', 1);
    //     ini_set('display_startup_errors', 1);
    //     error_reporting(E_ALL);
    ...
    //         0 => 'studies.created_at',
    //         1 => 'studies.accession',
    //         2 => 'analyst_name', // Use the alias directly
    //         3 => 'second_analyst_name', // Use the alias directly
    //         4 => 'studies.client_site_name',
    //         5 => 'clients.client_number',
    //         6 => 'client_details.site_code',
    //         7 => 'analyses_performed.analysis_performed',
    //         8 => 'analyses_client_price_details.analysis_code',
    //         9 => 'analyses.analysis_number',
    //         10 => 'studies.status_ids',
    //         11 => 'studies.completed_time'
    //     );
    ...
    //         cd.site_code, 
    //         c.client_number,
    //         ap.analysis_client_price_ids,
    //         ap.analysis_performed,
    //         acpd.analysis_id,
    //         acpd.analysis_code,
    //         a.analysis_number,
    //         ua.user_name as analyst_name,
    //         usb.user_name as second_analyst_name
    //     FROM studies s
    //     JOIN client_details cd ON cd.client_account_id = '$client_accid'
    //     JOIN clients c ON cd.client_ids = c.client_id
    //     JOIN analyses_performed ap ON s.studies_id = ap.studies_ids
    //     JOIN analyses_client_price_details acpd ON ap.analysis_client_price_ids = acpd.analysis_client_price_id
    //     JOIN analyses a ON acpd.analysis_id = a.analysis_id
    //     JOIN users ua ON s.analyst_id = ua.user_id
    //     JOIN users usb ON s.second_analyst_id = usb.user_id
    //     WHERE s.client_account_ids = '$client_accid'";
    ...
    //     if (!empty($request['search']['value'])) {
    //         $searchValue = mysqli_real_escape_string($con, $request['search']['value']);
    //         $sql .= " AND (s.accession LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR s.client_site_name LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR s.created_at LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR s.completed_time LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR a.analysis_number LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR a.analysis_code LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR c.client_number LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR ua.user_name LIKE '%" . $searchValue . "%' ";
    //         $sql .= " OR usb.user_name LIKE '%" . $searchValue . "%' )";
    //     }

    //     // First query to get total filtered records
    //     $query = mysqli_query($con, $sql);
    //     $totalData = mysqli_num_rows($query);
    //     $totalFilter = $totalData;

    //     // Add ordering and limit for pagination
    //     $sql .= " ORDER BY s.studies_id " . $request['order'][0]['dir'] . " LIMIT " . $request['start'] . ", " . $request['length'];
    ...
    //         // Fetch Analyst Names from query.
    //         $subdata[] = $row['analyst_name'];
    //         $subdata[] = $row['second_analyst_name'];
    ...
    //         // Fetch Analysis Details
    //         $subdata[] = $row['analysis_performed'];
    //         $subdata[] = $row['analysis_code'];
