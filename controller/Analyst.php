...
public function analyst_dicom_details_my() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['studies_id'])) {
        ...
        $update_success = $this->Admindb->update_study_details(
            $studies_id,
            $second_analyst_id,
            $status_ids,
            $analysis_array
        ...
        $_SESSION['alert'] = [
            'type' => $update_success ? 'success' : 'danger',
            'message' => $update_success ? 'Study updated successfully!' : 'Failed to update study.'
        ];

        $this->redirect('analyst/analyst_dicom_details_all');
    }

    // Handle single study view/edit (GET)
    if (isset($_GET['view']) && is_numeric($_GET['view'])) {
        ...
        $data['other_users'] = $this->Admindb->get_all_analyst($current_user_id); // 🛠️ Pass current user ID
    ...
    // Default list view
    $data['my_studies'] = $this->Admindb->get_studies_by_analyst($current_user_id);
    $data['asignee'] = $this->Admindb->get_all_analyst($current_user_id); // 🛠️ Fix here too
    $data['analysis_statuses'] = $this->Admindb->get_all_analysis_statuses();
...
public function add_new_study() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $client_site_name   = trim($_POST['client_site_name'] ?? '');
        $comment            = trim($_POST['comment'] ?? '');
        $client_account_ids = trim($_POST['client_account_ids'] ?? '');
        ...
        if (
            ...
            $this->add_alert('danger', 'All required fields must be filled.');
        } else {
            $study = [
                'accession' => $accession,
                'mrn' => $mrn,
            ...
            if ($result['type'] === 'success') {
                $this->add_alert('success', $result['msg']);
                $this->redirect('analyst/add_new_study');
                return;
            } else {
                $this->add_alert('danger', $result['msg']);
            }
        }
    }
        ...
        if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                $update = [
                    ...
                    'analysis_invoicing_description' => trim($_POST['analysis_invoicing_description'] ?? ''),
...
public function add_miscellaneous_billing() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        error_log("POST Billing Data: " . json_encode($billing));
        ...
        if ($result) {
            ...
            $this->redirect('analyst/add_miscellaneous_billing');
        } else {
            $this->add_alert('danger', 'Failed to add billing entry.');
            error_log("Insert failed in add_miscellaneous_billing");
        }
    }
...
public function study() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
            ...
            $tat = isset($_POST['tat']) ? trim($_POST['tat']) : '0';
            ...
            if ($status['type'] === 'success') {
                ...
                if ($customer_id) {
                    // Passing empty array for analysis_performed, as it is required param but empty here
                    $update_result = $this->Admindb->update_study_details($studies_id, null, 3, [], ['client_account_ids' => $customer_id]);
        ...
        if (isset($_GET['view']) && is_numeric($_GET['view'])) {
            ...
            if (!empty($study['analyst_id']) && $study['analyst_id'] != $this->user->user_id && !isset($_POST['reasn'])) {
...
public function current_month_studies()
{
    ...
    // ✅ FIXED: Pass current user ID to all calls to get_all_analyst()
    $data['asignee'] = $this->Admindb->get_all_analyst($current_user_id);
    $data['analysis_statuses'] = $this->Admindb->get_all_analysis_statuses();
