...
class Admintat extends Controller
{
    ...
    public function get_turnaround_time_info()
    {
        ...
        // Search query
        $sql = "SELECT tat_id, tat, tat_unit, tat_in_minutes, is_active, created_at FROM tat_master WHERE is_deleted != '1'";
        if (!empty($search_str)) {
        ...
        //     $order_col_index = (int)$request['order'][0]['column'];
        //     $order_dir = $request['order'][0]['dir'];

        //     if (isset($col[$order_col_index]) && in_array(strtolower($order_dir), ['asc', 'desc'])) {
        //         $sql .= " ORDER BY " . $col[$order_col_index] . " " . $order_dir;
        //     }
        // } else {
        //     // Default order if no user sort (initial load)
        //     $sql .= " ORDER BY created_at DESC";
        // }
    ...
    public function get_analyses()
    {
        ...
        AND
        COALESCE(acpd.is_deleted, a.is_deleted) != '1'";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $data[] = [
                ...
                'price' => $row['analysis_price'],
                'number' => $row['analysis_number']
            ];
        }
    ...
    //             // echo json_encode("Yes we entered the first parent");
    //             // return;


    //             // Flaw fixing code in discount
    //             $acpd_id = $this->Tatdb->get_acpdid_by_client($client_account_id, $analysis_value);
    ...
    //                 if (!$stmt1) {
    //                     die(json_encode(['status' => 'error', 'message' => 'Prepare failed for first UPDATE in parent->client', 'error' => $conn->error]));
    //                 }

    //                 $stmt1->bind_param("sssss", $analysis_name, $price, $itm_number, $acpd_id, $client_account_id);
    ...
    //                     if (!$stmt2) {
    //                         die(json_encode(['status' => 'error', 'message' => 'Prepare failed for second INSERT in parent->client', 'error' => $conn->error]));
    //                     }

    //                     $stmt2->bind_param("ssssss", $client_account_id, $acpd_id, $frm_value, $to_value, $price, $is_active);

    //                     if ($stmt2->execute()) {
    //                         echo json_encode(['status' => 'success', 'message' => 'Both Updation and Insertion done successfully in parent->client']);
    //                     } else {
    //                         echo json_encode(['status' => 'error', 'message' => 'Insertion failed in second query in parent->client', 'error' => $stmt2->error]);
    //                     }
    //                 } else {
    //                     echo json_encode(['status' => 'error', 'message' => 'Updation failed in first query of parent->client', 'error' => $stmt1->error]);
    //                 }
    //             } else if (!$acpd_id || $acpd_id == false) {
    ...
    //                 $stmt1->bind_param("ssssss", $client_account_id, $analysis_value, $analysis_name, $price, $itm_number, $is_active);
    //                 if ($stmt1->execute()) {
    ...
    //                     $stmt2->bind_param("ssssss", $client_account_id, $analysis_client_price_id, $frm_value, $to_value, $price, $is_active);
    ...
    //             $stmt1->bind_param("ssss", $analysis_name, $price, $itm_number, $analysis_value, $client_account_id);
    ...
    //                 $stmt2->bind_param("ssssss", $client_account_id, $analysis_value, $frm_value, $to_value, $price, $is_active);

    //                 if ($stmt2->execute()) {
    //                     echo json_encode(['status' => 'success', 'message' => 'Both Updation and Insertion done successfully']);
    //                 } else {
    //                     echo json_encode(['status' => 'error', 'message' => 'Insertion failed in second query in client', 'error' => $stmt2->error]);
    //                 }
    //             } else {
    //                 echo json_encode(['status' => 'error', 'message' => 'Updation failed in first query', 'error' => $stmt1->error]);
    //             }
    //         }
    //     } else {
    ...
    public function add_to_disclist()
    {
        ...
        if (empty($table_id)) {
            echo json_encode(['status' => 'error', 'message' => 'Missing table ID']);
            return;
        }
        ...
        if ($table_id == "parent") {
            ...
            if ($acpd_id) {
                $sql1 = "UPDATE analyses_client_price_details
                     SET analysis_name = ?, analysis_client_price = ?, analysis_code = ?
                     WHERE analysis_client_price_id = ? AND client_account_ids = ?";

                $stmt1 = $conn->prepare($sql1);
                if (!$stmt1) {
                    echo json_encode(['status' => 'error', 'message' => 'Prepare failed for parent update', 'error' => $conn->error]);
                    return;
                }
                ...
                if ($stmt1->execute()) {
                    $sql2 = "INSERT INTO monthly_volume_discount
                         (client_account_ids, analysis_client_price_ids, minimum_volume, maximum_volume, discount_price, is_active)
                         VALUES (?, ?, ?, ?, ?, ?)";

                    $stmt2 = $conn->prepare($sql2);
                    if (!$stmt2) {
                        echo json_encode(['status' => 'error', 'message' => 'Prepare failed for parent insert', 'error' => $conn->error]);
                        return;
                    }

                    $stmt2->bind_param("ssssss", $client_account_id, $acpd_id, $frm_value, $to_value, $price, $is_active);
                    if ($stmt2->execute()) {
                        echo json_encode(['status' => 'success', 'message' => 'Parent updated and discount added.']);
                    } else {
                        echo json_encode(['status' => 'error', 'message' => 'Failed to insert discount (parent)', 'error' => $stmt2->error]);
                    }
                } else {
                    echo json_encode(['status' => 'error', 'message' => 'Failed to update analysis (parent)', 'error' => $stmt1->error]);
                }
            } else {
                // INSERT both analysis and discount
                $sql1 = "INSERT INTO analyses_client_price_details 
                     (client_account_ids, analysis_id, analysis_name, analysis_client_price, analysis_code, is_active)
                     VALUES (?, ?, ?, ?, ?, ?)";

                $stmt1 = $conn->prepare($sql1);
                if (!$stmt1) {
                    echo json_encode(['status' => 'error', 'message' => 'Prepare failed for insert analysis (parent)', 'error' => $conn->error]);
                    return;
                }
                ...
                if ($stmt1->execute()) {
                    ...
                    $stmt2 = $conn->prepare($sql2);
                    if (!$stmt2) {
                        echo json_encode(['status' => 'error', 'message' => 'Prepare failed for insert discount (parent)', 'error' => $conn->error]);
                        return;
                    }

                    $stmt2->bind_param("ssssss", $client_account_id, $analysis_client_price_id, $frm_value, $to_value, $price, $is_active);
                    if ($stmt2->execute()) {
                        echo json_encode(['status' => 'success', 'message' => 'Parent analysis and discount inserted.']);
                    } else {
                        echo json_encode(['status' => 'error', 'message' => 'Failed to insert discount (new parent)', 'error' => $stmt2->error]);
                    }
                } else {
                    echo json_encode(['status' => 'error', 'message' => 'Failed to insert analysis (new parent)', 'error' => $stmt1->error]);
                }
            }
            ...
            $stmt1 = $conn->prepare($sql1);
            if (!$stmt1) {
                echo json_encode(['status' => 'error', 'message' => 'Prepare failed for update (client)', 'error' => $conn->error]);
                return;
            }

            $stmt1->bind_param("sssss", $analysis_name, $price, $itm_number, $analysis_value, $client_account_id);
            if ($stmt1->execute()) {
                $sql2 = "INSERT INTO monthly_volume_discount
                     (client_account_ids, analysis_client_price_ids, minimum_volume, maximum_volume, discount_price, is_active)
                     VALUES (?, ?, ?, ?, ?, ?)";
                ...
                $stmt2 = $conn->prepare($sql2);
                if (!$stmt2) {
                    echo json_encode(['status' => 'error', 'message' => 'Prepare failed for insert discount (client)', 'error' => $conn->error]);
                    return;
                }

                $stmt2->bind_param("ssssss", $client_account_id, $analysis_value, $frm_value, $to_value, $price, $is_active);
                if ($stmt2->execute()) {
                    echo json_encode(['status' => 'success', 'message' => 'Client analysis updated and discount added.']);
                } else {
                    echo json_encode(['status' => 'error', 'message' => 'Failed to insert discount (client)', 'error' => $stmt2->error]);
                }
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Failed to update analysis (client)', 'error' => $stmt1->error]);
            }
        }
    }
    ...
    public function update_analysis_monthly_discount()
    {
        ...
        if (
            !isset($price_id, $client_acc_id, $disc_id, $analysis_name, $analysis_from, $analysis_to, $analysis_price) ||
            $price_id === '' ||
            $client_acc_id === '' ||
            $disc_id === '' ||
            $analysis_name === '' ||
        ...
        $stmt1->bind_param("ssss", $analysis_name, $analysis_price, $price_id, $client_acc_id);
        ...
        if ($stmt1->execute()) {
            ...
            $stmt2->bind_param("ssssss", $analysis_from, $analysis_to, $analysis_price, $disc_id, $client_acc_id, $price_id);
    ...
    public function analyses_for_subscription()
    {
        $conn = $this->getConnection();
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $data[] = [
                'value' => !empty($row['analysis_client_price_id']) ? $row['analysis_client_price_id'] : $row['analysis_id'],
                'name'  => $row['analysis_name'],
                'description'  => $row['analysis_invoicing_description'],
                'table' => !empty($row['analysis_client_price_id']) ? "client" : "parent",
                'price' => $row['analysis_price'],
                'anumber' => $row['analysis_number']
            ];
        }
    ...
    public function subscribed_analyses()
    {
        $conn = $this->getConnection();
        ...
        $sql = "SELECT t2.subscription_ids,
                       t1.subscription_price,
                       t2.subscription_content_id,
                       t2.analysis_client_price_ids,
                       t2.subscription_volume,
                       t3.analysis_name,
                       t3.analysis_code 
                       FROM subscription t1 
                       INNER JOIN subscription_contents t2 ON (t1.subscription_id = t2.subscription_ids) 
                       INNER JOIN analyses_client_price_details t3 
                       ON (t2.analysis_client_price_ids = t3.analysis_client_price_id) 
                       WHERE t1.client_account_ids = ? 
                       AND t2.is_deleted = '0' 
                       AND t1.is_deleted = '0'
                       ORDER BY t2.subscription_content_id DESC";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $subdata[] = $row['analysis_code'];
            $subdata[] = $row['analysis_name'];
            $subdata[] = $row['subscription_volume'];
            $subdata[] = $row['subscription_price'];
            $subdata[] = $row['subscription_ids'];
            $subdata[] = $row['analysis_client_price_ids'];
            $subdata[] = $row['subscription_content_id'];
            $data[] = $subdata;
        }
    ...
    //     $client_acc_id = $_POST['client_account_id'];
    //     $client_sub_id = $_POST['client_sub_id'];
    //     $analysisName = $_POST['subanalysname'];
    //     $analysis_id = $_POST['subanalysvalue'];
    //     $table_id = $_POST['dtable_id'];
    //     $analysis_price = $_POST['data_aprice'];
    //     $analysis_desc = $_POST['data_adesc'];
    //     $itemNumber = $_POST['data_anumber'];
    //     $count = $_POST['data_scount'];
    //     $created_by = $_SESSION['user']->user_id;

    //     if ($client_sub_id == '0') {
    //         $sub_amount = 0;
    //         $subscription_id = $this->Tatdb->add_subscription_id($client_acc_id, $sub_amount, $created_by);

    //         $sub_success = $this->Tatdb->save_added_subscriptions($client_acc_id, $subscription_id, $created_by, $analysis_id, $table_id, $analysis_price, $analysis_desc, $itemNumber, $analysisName, $count);

    //         if (!$sub_success) {
    //             echo json_encode([
    //                 'success' => false,
    //                 'message' => 'Error saving one of the subscriptions.'
    //             ]);
    //             return;
    //         }
    //     } else {
    //         $subscription_id = $client_sub_id;
    //         $sub_success = $this->Tatdb->save_added_subscriptions($client_acc_id, $subscription_id, $created_by, $analysis_id, $table_id, $analysis_price, $analysis_desc, $itemNumber, $analysisName, $count);
    ...
    public function save_subscriptions_added()
    {
        $conn = $this->getConnection();
        ...
        $client_acc_id = $_POST['clientid'] ?? '';
        $sub_amount = $_POST['subscription_amount'] ?? '';
        $subscriptions_json = $_POST['subscriptions'] ?? '[]';
        //$subscriptions = json_decode($subscriptions_json, true);
        $subscriptions = $_POST['subscriptions'] ?? [];
        $created_by = $_SESSION['user']->user_id;

        // if (empty($client_acc_id) || empty($sub_amount) || empty($subscriptions)) {
        if (empty($client_acc_id) || empty($sub_amount)) {
            echo json_encode([
                'success' => false,
        ...
        $check_query = "SELECT subscription_id FROM subscription WHERE client_account_ids = '$client_acc_id' AND is_deleted != '1' AND created_by = '$created_by'";
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            $sub_amount_update = $this->Tatdb->update_subscription_amount($client_acc_id, $subscription_id, $sub_amount, $created_by);

            if (!$sub_amount_update) {
                echo json_encode([
                    'success' => false,
                    'message' => 'Error updating subscription amount.'
                ]);
                return;
            }
            ...
            if (!empty($subscriptions)) {
                ...
                foreach ($subscriptions as $sub) {
                    ...
                    $analysis_price = $sub['analysis_price'];
                    $analysis_desc = $sub['a_desc'];
                    $itemNumber = $sub['item_number'];
                    $analysisName = $sub['analysis_name'];
                    $count = $sub['count'];

                    // $sub_success = $this->Tatdb->save_added_subscriptions($client_acc_id, $sub_amount, $created_by, $analysis_id, $table_id, $analysis_price, $analysis_desc, $itemNumber, $analysisName, $count);
                    $sub_success = $this->Tatdb->save_added_subscriptions($client_acc_id, $subscription_id, $created_by, $analysis_id, $table_id, $analysis_price, $analysis_desc, $itemNumber, $analysisName, $count);
    ...
    public function check_analysis_subscription()
    {
        // header('Content-Type: application/json');
        // echo json_encode($_POST);
        // return;
        ...
        $analysis_id = $_POST['analysis_id'] ?? null;

        if (!$client_id || !$analysis_id) {
            echo json_encode(['exists' => false, 'error' => 'Missing data']);
            return;
        }

        // Get subscription_id from client_id
        $subscription_id = $this->Tatdb->get_subscription_id_by_client($client_id);

        if (!$subscription_id) {
            echo json_encode(['exists' => false, 'error' => 'No subscription found']);
            return;
        }

        $exists = $this->Tatdb->is_analysis_subscribed($subscription_id, $analysis_id);

        echo json_encode(['exists' => $exists]);
    }

    public function update_analysis_subscription()
    {
        // header('Content-Type: application/json');
        // echo json_encode($_POST);
        // return;
        ...
        $client_acc_id = $_POST['client_acc_id'];
        $acpd_id = $_POST['acpd_id'];
        $sub_id = $_POST['sub_id'];
        $subct_id = $_POST['subct_id'];
        $subanalysis_name = $_POST['subanalysis_name'];
        $subanalysis_count = $_POST['subanalysis_count'];

        $update_acpd = $this->Tatdb->update_susbscribed_analysis($acpd_id, $client_acc_id, $subanalysis_name);

        $update_subcount = $this->Tatdb->update_sub_count($subct_id, $sub_id, $acpd_id, $subanalysis_count);
    }

    public function delete_analysis_subscription()
    {
        // header('Content-Type: application/json');
        // echo json_encode($_POST);
        // return;
        ...
        $acpd_id = $_POST['acpd_id'];
        $sub_id = $_POST['sub_id'];
        $subct_id = $_POST['subctId'];

        $delete_subscription = $this->Tatdb->delete_analysis_subscription($subct_id, $sub_id, $acpd_id);
    }

    public function analyses_for_maintenance()
    {
        $conn = $this->getConnection();
    ...
    public function add_to_monthlyfee()
    {
        // header('Content-Type: application/json');

        $required_fields = ['client_account_id', 'analysname', 'analysvalue', 'table_id', 'a_number', 'def_price', 'monthly_fee'];
        ...
        $client_acc_id = $_POST['client_account_id'];
        $analysisName = $_POST['analysname'];
        $analysis_id = $_POST['analysvalue'];
        $table_id = $_POST['table_id'];
        $itemNumber = $_POST['a_number'];
        $analysis_price = $_POST['def_price'];
        $analysis_desc = $_POST['a_desc'];
        ...
        $monthly_fee_status = $this->Tatdb->add_new_monthly_fees(
            $client_acc_id,
            $analysisName,
            $analysis_id,
            $table_id,
        ...
        if ($monthly_fee_status === true) {
            ...
            echo json_encode([
                'status' => 'error',
                'message' => $monthly_fee_status
            ]);
        }
    }

    public function analyses_mainfee_list()
    {
        $conn = $this->getConnection();
        ...
        $sql = "SELECT acpd.analysis_client_price_id,
                       acpd.client_account_ids,
                       acpd.analysis_id,
                       acpd.analysis_name,
                       acpd.analysis_client_price,
                       acpd.analysis_code,
                       mfee.maintenance_fees_id,
                       mfee.client_account_ids,
                       mfee.analysis_client_price_ids,
                       mfee.maintenance_fee_amount
                       FROM analyses_client_price_details acpd
                       INNER JOIN maintenance_fees mfee ON (acpd.analysis_client_price_id = mfee.analysis_client_price_ids AND acpd.client_account_ids = mfee.client_account_ids)
                       WHERE acpd.client_account_ids = ?
                       AND mfee.is_deleted = '0'
                       ORDER BY mfee.maintenance_fees_id DESC";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $subdata[] = $row['analysis_code'];
            $subdata[] = $row['analysis_name'];
            $subdata[] = $row['analysis_client_price'];
            $subdata[] = $row['maintenance_fee_amount'];
            $subdata[] = $row['analysis_client_price_ids'];
            $subdata[] = $row['maintenance_fees_id'];
            // $subdata[] = $row['subscription_ids'];
            // $subdata[] = $row['analysis_client_price_ids'];
            // $subdata[] = $row['subscription_content_id'];
            $data[] = $subdata;
    ...
    public function update_analysis_maintenance_fee()
    {
        $conn = $this->getConnection();
        ...
        $aprice_id = $_POST['aprice_id'] ?? '';
        $mainfee_id = $_POST['mainfee_id'] ?? '';
        $client_acc_id = $_POST['client_acc_id'] ?? '';
        $amname = $_POST['analysis_mname'] ?? '';
        $acprice = $_POST['analysis_cprice'] ?? '';
        $amprice = $_POST['analysis_mprice'] ?? '';

        if (
            !isset($aprice_id, $mainfee_id, $client_acc_id, $amname, $acprice, $amprice) ||
            $aprice_id === '' ||
            $mainfee_id === '' ||
            $client_acc_id === '' ||
            $amname === '' ||
        ...
        $update_status = $this->Tatdb->update_analysis_maintenance_fees($aprice_id, $mainfee_id, $client_acc_id, $amname, $acprice, $amprice);
    ...
    public function delete_analysis_maintenance_fee()
    {
        $conn = $this->getConnection();
        ...
        if (
            !isset($aprice_id, $mainfee_id, $client_acc_id) ||
            $aprice_id === '' ||
            $mainfee_id === '' ||
            $client_acc_id === ''
        ) {
        ...
        $delete_maintenance_fee_status = $this->Tatdb->delete_maintenancefee($aprice_id, $mainfee_id, $client_acc_id);

        if ($delete_maintenance_fee_status === true) {
            echo json_encode([
                'status' => 'success',
            ...
            echo json_encode([
                'status' => 'error',
                'message' => $delete_maintenance_fee_status
            ]);
        }
    }
