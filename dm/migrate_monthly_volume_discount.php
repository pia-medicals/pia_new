...
try {
    ...
    $oldData = $old_db->query("SELECT * FROM discount_range");
    ...
    if ($oldData->num_rows > 0) {
        ...
        while ($row = $oldData->fetch_assoc()) {
            ...
            $stmt = $new_db->prepare("INSERT INTO monthly_volume_discount (discount_id,client_account_ids,minimum_volume,maximum_volume,discount_price,is_active,created_at) VALUES (?, ?, ?, ?,?, ?, NOW())");
            $stmt->bind_param("iiiiss", $row['id'],$client_account_ids,$row['minimum_value'],$row['maximum_value'],$row['percentage'],$is_active);
