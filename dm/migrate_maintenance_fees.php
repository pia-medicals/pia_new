...
try {
    ...
    if ($oldData->num_rows > 0) {
        ...
        while ($row = $oldData->fetch_assoc()) {
            ...
            $stmt = $new_db->prepare("INSERT INTO maintenance_fees (maintenance_fees_id,client_account_ids,maintenance_fee_type,maintenance_fee_amount,is_active,created_at) VALUES (?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("iisis", $row['id'],$client_account_ids,$row['maintenance_fee_type'],$row['maintenance_fee_amount'],$is_active);

            if ($stmt->execute()) {
                $insertedCount++; // Increment count if insertion is successful
            } else {
                throw new Exception("Failed to insert data into maintenance_fees table: " . $row['id'] ."/n". $stmt->error);
            }
        }
