    ...
    foreach ($site as $kk) {
        ...
        <?php if (!empty($wsheet)) { ?>
                    ...
                    <?php foreach ($wsheet[$kk] as $key => $wsheets) { ?>
                        ...
                        $previous_carry = !empty($previous_carrys['count']) ? $previous_carrys['count'] : 0;
                        ?>
...
//echo $wsheets['ans_id']; 
                                ...
                                <td>
                                    ...
                                    print_r($sub_count['count']); ?>
                                </td>
                                <td><?php echo $previous_carry; ?></td>
                                <td><?php $total_balance = $sub_count['count'] + $previous_carry;
                                    echo $total_balance; ?></td>
        ...
        <?php } ?>   
            ...
            if (isset($site)) {
                ...
                $time_ids = !empty($new_time_id[$kk]['time_id']) ? $new_time_id[$kk]['time_id'] : '';


                $sub_amount = $this->Admindb->get_subscription_by_time_id($kk, $time_ids);

                $sb_amount = !empty($sub_amount['subscription_fees']) ? $sub_amount['subscription_fees'] : 0;
                ?><tfoot>
        ...
        <table class="admin">
            ...
            <tbody>
                ...
                if (!$sub_found) {
                    ?>
                            <tr>
                                ...
                                <td>
                                    <?php $analysis_name = $this->Admindb->analyses_name_by_id($wsheets['ans_id']);
                                    echo $analysis_name; //echo $wsheets['ans_id'];   ?>
                                </td>
                                <td>
                    <?php echo $wsheets['rate']; ?>
                                </td>
                                <td>
                    <?php $extra_count += $wsheets['qty'];
                    echo $wsheets['qty']; ?>
                                </td>
                                <td>
                            <?php $extra_amount += $wsheets['qty'] * $wsheets['rate'];
                            echo $wsheets['qty'] * $wsheets['rate']; ?>
                                </td>
                ...
                <?php } else { ?>
                                    ...
                                    <?php
                                    $sub_count = $this->Admindb->count_subscription($kk, $wsheets['ans_id'], $time_id[$kk][0]);
                                    $total_balance = $sub_count['count'] + $previous_carry;
                                    $balance_carry = $total_balance - $wsheets['qty'];
                                    if ($balance_carry < 0) {
                                        ?> <!-- show only - value counts -->
                                <tr>
                        ...
                        <?php echo $wsheets['rate']; ?>
                                    ...
                                    <td>
                                    <?php $extra_count += $balance_carry * -1;
                                    echo $balance_carry * -1; ?>
                                    </td>

                                    <td>
                        <?php $extra_amount += ($balance_carry * -1) * $wsheets['rate'];
                        echo ($balance_carry * -1) * $wsheets['rate']; ?>
                                    </td>
        ...
        <?php if (!empty($wsheet)) { ?>
                <tr>
                    ...
                    <td id="disc">Discount:
                        <strong><?php $discount = $extra_amount * $discount_range['percentage'] / 100;
                    echo $discount; ?></strong>
                    </td>
                    <td id="t_amt_aftr">Total amount after <span>(<?php echo $discount_range['percentage'] ?> </span>%) Discount: 
                        <strong><?php $total_after_disc = $extra_amount - $discount;
                    echo $total_after_disc; ?></strong>
                    </td>
                </tr>
                <tr>
                ...
                <?php 
                $maintenance_fee_amount = !empty($maintenance['maintenance_fee_amount']) ? $maintenance['maintenance_fee_amount'] : 0;
                $grand_total = $total_after_disc + $maintenance['maintenance_fee_amount'] + $sb_amount; ?>
                    <td id="gtotal" style="font-size: 18px; background: #cbd7d3;">Grand Total : <strong><?php echo $grand_total; ?></strong></td>
                </tr>
        <?php } else { ?>
                <tr>
            <?php if (!empty($site) && !empty($time_id[$kk][0])) { ?>
                <?php                
                $maintenance = $this->Admindb->get_maintenance_by_customer_and_time_Id($kk, $time_id[$kk][0]);                
                ?>
                        <td id="main_fee_amt">Maintenance fee amount: <span><?php echo $maintenance['maintenance_fee_amount']; ?></span></td>
                        <td id="main_fee_type">Maintenance fee type: <span><?php echo $maintenance['maintenance_fee_type']; ?></span></td>
                <?php
                ...
                $maintenance_fee_amount = !empty($maintenance['maintenance_fee_amount']) ? $maintenance['maintenance_fee_amount'] : 0;
                $grand_total = $maintenance_fee_amount + $sb_amount; 
                ?>
                        <td id="gtotal" style="font-size: 18px; background: #cbd7d3;">Grand Total : <strong><?php echo $grand_total; ?></strong></td>
            <?php } ?> 
                </tr> 
