		...
		<div class="box box-primary fl100">
			...
			<div class="col-md-12">
                <table class="table table-bordered">
                	<tbody>
                        ...
                        <tr>
                            <td>Status</td>
                            <td><?=$this->get_worksheet_status($edit['status']) ?></td>
                        </tr>
                        
                        <?php if(isset($edit['status']) && $edit['status'] == 'Completed'): ?>
                        <tr>
                            <td>Completed Time</td>
                            <td><?=$edit_wsheet['completed_time'] ?></td>
                        </tr>
                        <?php endif; ?>
                        <tr>
                            <td>Priority</td>
			...
			<form role="Form" method="post" action="" class="admin_form" accept-charset="UTF-8" autocomplete="off" data-form-validate="true" novalidate="novalidate">
				<div class="form-group">
					...
					<select  id="analyst" name="analyst" class="form-control analyst_choose select2" required >
				...
				<div class="form-group">
					...
					<select class="form-control select2" id="review-analyst" name="txtReviewAnalyst">
				...
				<div class="form-group">
                    ...
                    <?php $status = array(
                            ...
                            'Completed'=>'Completed',
                            'Cancelled'=>'Cancelled - duplicate order',
                            'CancelledAcc'=>'Cancelled - accidental send',
                            'CancelledCust'=>'Cancelled - by customer during analysis',
                            'On hold'=>'On hold'
                        );
                    ...
                    <?php 
                        foreach ($status as $key => $value) {
                            if($key == $act_status) $sel_st = 'selected'; else $sel_st = '';
                            echo '<option value="'.$key.'" '.$sel_st.' >'.$value.'</option>';
                        }
                    ?>
                    </select>
                </div>
				...
				<div class="form-group analyses_select">
                    ...
                    <div class="form-group col-xs-6 analyses_per" id="analyses_performed_new">
                    	<select class="form-control select2" id="analyses_performed_sel">
                            <option value="">Choose Analysis Performed</option>
                    		<?php foreach ($billing_codes as $key => $value) {
                    			//echo '<option value="'.$value['id'].'">'.$value['custom_description'].'</option>';
...
//print_r($analyses_ids);
//print_r($analyses_performed);
?>
                <div class="form-group">
                    <table class="table table-bordered" id="ans-table">
                        <tbody id="ans-table">
                            ...
                            <tr id="row-<?php echo $analyses_ids[$i];?>">
                                <td><?php echo $this->get_analyses_name($analyses_ids[$i])//echo $analyses_performed[$i];?>&emsp;<input type="hidden" value="<?php echo $analyses_ids[$i];?>" name="analyses_performed[]"/><input type="hidden" name="ans_hr[]" placeholder="Analyses Time(Minute)" value="<?php echo $any_mint[$i];?>" /></td>
                        		
                        		<td width="20%"><button type="button" class="btn btn-block btn-danger delete-row" id="delete-<?php echo $analyses_ids[$i];?>">Remove</button></td>
                        	</tr>	
                        	<?php  } } ?>
                        </tbody>
                    </table>
				...
				<div class="form-group remove_assignee" style="display: none;">
                ...
                <div class="form-group remove_assignee_worksheet" style="display: none;">
                    <input type="hidden" name="wsheet_id" data-rule-required="true" value="<?php issetecho($edit_wsheet['id'])?>">
...
$('#add-ans').on('click',function(){
	...
	if($('#row-'+analyses_performed_id).length){
		...
		var tablerow					=	'<tr id="row-'+analyses_performed_id+'"><td>'+analyses_performed_text+'&emsp;<input type="hidden" value="'+analyses_performed_id+'" name="analyses_performed[]"/><input type="hidden" name="ans_hr[]" value="0" placeholder="Analyses Time(Minute)" value=""/></td><td width="20%"><button type="button" class="btn btn-block btn-danger delete-row" id="delete-'+analyses_performed_id+'">Remove</button></td></tr>';
...
function show_submit_button(){
    var stausVal        =   $('#status').val();
    var secondCheck     =   $('#review-analyst option:selected').val();
    var hdnlength = $('#ans-table input:hidden').length;
    //alert(secondCheck);
    if(((secondCheck=='')&&(stausVal=='Completed')) || ((hdnlength==0)&&(stausVal=='Completed')) ){
...
$('#status').on('change',function(){
    ...
    show_submit_button();
       var stausVal     =   $(this).val(); 
       if(stausVal=='CancelledAcc'){
            if (confirm('This will trigger a $10 charge in billing.Are you sure you want to continue with this?')) {
                
            } else {
                $('#status').val('<?php echo $act_status ?>');
            }
       }

       if(stausVal=='CancelledCust'){
            if (confirm('This will trigger a $25 charge in billing.Are you sure you want to continue with this?')) {
                
            } else {
                $('#status').val('<?php echo $act_status ?>');
            }
       }
       if(stausVal=='Completed'){
        $('#review-analyst').attr('data-rule-required', true);
        $('#analyses_performed_sel').attr('data-rule-required', true);
       } else {
        $('#review-analyst').removeAttr( "data-rule-required" );
        $('#analyses_performed_sel').removeAttr( "data-rule-required" );
       }
        dispRemoveAssignee(stausVal);
});
...
var sts = $("#status").val();
dispRemoveAssignee(sts);

function dispRemoveAssignee(status){    
    if(status=='Cancelled'){
        $('.remove_assignee').show();
        $('.remove_assignee_worksheet').show();
    }
    else{
        $('.remove_assignee').hide();
        $('.remove_assignee_worksheet').hide();
    }
}
