                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select id="tat_unit" class="form-control" name="tat_unit" required>
                                            <?php
                                            $tat = $edit['tat'];
                                            $tat_unit = $edit['tat_unit'];

                                            // Normalize units if TAT is 1
                                            if ($tat == "1") {
                                                if ($tat_unit == "Hours") {
                                                    $tat_unit = "Hour";
                                                } elseif ($tat_unit == "Minutes") {
                                                    $tat_unit = "Minute";
                                                }
                                            }

                                            $hours_label = ($tat == "1") ? "Hour" : "Hours";
                                            $minutes_label = ($tat == "1") ? "Minute" : "Minutes";
                                            ?>
                                            ...
                                            <?php /* <option value="Hours" <?php if ($edit['tat_unit'] == "Hours") echo "selected"; ?>>Hours</option>
                                            <option value="Minutes" <?php if ($edit['tat_unit'] == "Minutes") echo "selected"; ?>>Minutes</option> */ ?>
                                            <option value="Hours" <?php if ($tat_unit == "Hour" || $tat_unit == "Hours") echo "selected"; ?>><?= $hours_label; ?></option>
                                            <option value="Minutes" <?php if ($tat_unit == "Minute" || $tat_unit == "Minutes") echo "selected"; ?>><?= $minutes_label; ?></option>
                                        </select>
                                        <input type="hidden" id="id" name="id" value="<?= $edit['tat_id'] ?>">
                                    </div>
                                    <?php /* <small class="form-text text-muted">Enter only 30 if choosing "Minutes". Maximum is 48 hours.</small> */ ?>
                                </div>
                                <div class="form-group">
                                    <label>Status</label>
                                    <div class="input-group">
...
<script>
    ...
    $('#tat_unit').on('change', function() {
        ...
        if (tatValue === '1' && tatUnitSelect.val() === 'Minutes') {
            if (tatUnitSelect.find('option[value="Minutes"]').length) {
                tatUnitSelect.find('option[value="Minutes"]').text('Minute');
            }
        } else {
            if (tatUnitSelect.find('option[value="Minutes"]').length) {
                tatUnitSelect.find('option[value="Minutes"]').text('Minutes');
