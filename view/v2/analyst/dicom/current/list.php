    ...
    <section class="content">
        ...
        <div class="container-fluid">
            <form method="get" action="" class="form-inline mb-3">
                ...
                <a href="<?= strtok($_SERVER["REQUEST_URI"], '?') ?>" class="btn btn-sm btn-secondary">Reset</a>
            </form>
                    ...
                    <tbody>
                        <?php if (!empty($open_studies)): ?>
                            <?php foreach ($open_studies as $row): ?>
                                <tr>
                                    <td>
                                        <?php if (!empty($row['created_at']) && strtotime($row['created_at']) !== false): ?>
                                    ...
                                    <td class="text-center action-icons">
                                        <a href="<?= SITE_URL ?>/analyst/analyst_dicom_details_my?view=<?= htmlspecialchars($row['studies_id'] ?? '') ?>" class="btn btn-info btn-sm"> <i class="fas fa-edit"></i>Edit</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="7" class="text-center">
                                    No studies found for <?= date('F Y', strtotime($year . '-' . $month . '-01')) ?>.
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
                 <div class="bottom-title">Current Month Studies</div>
            </div> <!-- /.card-body -->
        </div> <!-- /.card -->
    </section>
</div>
